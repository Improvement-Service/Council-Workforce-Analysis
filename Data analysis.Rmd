...
``` {r aggregate Scotland data, echo = FALSE}
...
create_graphs <- function(graph_characteristic) {
  filtered_data <- scotland_data %>% filter(Characteristic == graph_characteristic)
  ggplot(filtered_data, aes(x = Measure, y = Percent)) +
